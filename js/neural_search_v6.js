...
// ==================== 📚 البحث داخل الأدلة (Guides Search Engine) ====================
async function handleGuideSearch(query, activeGuideInfo) {
    ...
    const normalizedQuery = advancedNormalize(query);
    const keywords = intelligentTokenize(query).tokens;
    ...
    // ✨ بناء مشتقات العبارة الدقيقة للبحث عنها كوحدة واحدة (مثل "المادة 50")
    let phrasePatterns = [normalizedQuery];
    const matchArticleNumber = query.match(/(مادة|المادة)\s*(رقم\s*)?(\d+)/i);
    if (matchArticleNumber) {
        const num = matchArticleNumber[3];
        phrasePatterns.push(advancedNormalize(`المادة ${num}`));
        phrasePatterns.push(advancedNormalize(`مادة ${num}`));
        phrasePatterns.push(advancedNormalize(`المادة رقم ${num}`));
    }

    const scoredPages = guide.pages.map(page => {
        let score = 0;
        let matches =[];
        let exactPhraseFound = null;
        const pageText = advancedNormalize(page.text);

        // 1. فحص العبارة الدقيقة الكاملة (أولوية مطلقة بـ 5000 نقطة)
        for (const phrase of phrasePatterns) {
            if (pageText.includes(phrase)) {
                score += 5000;
                exactPhraseFound = phrase;
                matches.push(phrase);
                break; // اكتفينا بالتطابق التام
            }
        }

        // 2. فحص الكلمات المتفرقة (داعم فقط)
        keywords.forEach(kw => {
            const isNum = /^\d+$/.test(kw);
            if (pageText.includes(kw)) {
                // الأرقام تأخذ وزن عالي جداً لتميزها
                score += isNum ? 500 : (kw.length * 2); 
                matches.push(kw);
            } else if (kw.length > 4) {
                 const subKw = kw.substring(0, Math.floor(kw.length * 0.7));
                 if (pageText.includes(subKw)) {
                     score += subKw.length;
                     matches.push(kw + "*");
                 }
            }
        ...
        return {
            ...
            exactPhraseFound: exactPhraseFound
        };
    });
    ...
    // ✨ نظام كشف الالتباس (Ambiguity Check)
    // هل وجدنا التطابق التام في أكثر من صفحة مختلفة؟
    const exactMatches = results.filter(r => r.score >= 5000);

    if (exactMatches.length > 1) {
        let clarifyHtml = `
        <div style="background: #fff3e0; border-left: 4px solid #ff9800; padding: 12px; margin-bottom: 8px; border-radius: 6px;">
            <div style="color: #e65100; font-weight: bold; margin-bottom: 8px;">
                🤔 عثرت على "${query}" في عدة مواضع، أيهم تقصد؟
            </div>
            <div style="display: flex; flex-direction: column; gap: 8px;">`;

        // عرض خيارات لأفضل 4 مواضع
        exactMatches.slice(0, 4).forEach((match) => {
            const shortExcerpt = getExcerpt(match.text, [match.exactPhraseFound], true);
            clarifyHtml += `
            <button class="choice-btn" onclick="window.showGuideSnippet('${guide.source_file}', ${match.page_num}, '${match.exactPhraseFound}')" style="text-align: right; padding: 10px; background: white; border: 1px solid #ffcc80; border-radius: 6px; cursor: pointer;">
                <div style="font-weight: bold; color: #d84315; margin-bottom: 4px;">👇 ذكرت في صفحة ${match.page_num}</div>
                <div style="font-size: 0.85em; color: #555;">"${shortExcerpt}"</div>
            </button>`;
        });
    ...
    // ✨ الحالة العادية: نتيجة واحدة قوية
    const topResult = results[0];
    const targetSearch = topResult.exactPhraseFound ? [topResult.exactPhraseFound] : keywords;
    const excerpt = getExcerpt(topResult.text, targetSearch, false);
    ...
    if (results.length > 1 && !exactMatches.length) {
        const otherPages = results.slice(1, 3).map(r => r.page_num).join(' و ');
        response += `
        <div style="margin-top: 8px; font-size: 0.8em; color: #6b7280; text-align: center;">
            💡 معلومات ذات صلة قد تكون في صفحة ${otherPages}
        </div>`;
    }
...
function getExcerpt(text, keywords, isShort = false) {
    ...
    // 2. إذا لم يجد عبارة، نبحث عن الأرقام كأولوية قصوى (مثل 50 أو 3)
    if (firstIndex === -1) {
        const numKeywords = keywords.filter(k => /^\d+$/.test(k.replace('*', '')));
    ...
    // 3. الخطوة الأخيرة: البحث عن أي كلمة عادية
    if (firstIndex === -1) {
    ...
    // تحديد طول الاقتباس بناءً على السياق
    const beforeLen = isShort ? 40 : 120;
    const afterLen = isShort ? 80 : 300;

    const start = Math.max(0, firstIndex - beforeLen);
    const end = Math.min(text.length, firstIndex + matchedTarget.length + afterLen);
    
    let snippet = text.substring(start, end);
    
    // تمييز الكلمات (Highlight) آمن برمجياً
    keywords.forEach(kw => {
        const cleanKw = kw.replace('*', '');
        if (cleanKw.length > 0) {
